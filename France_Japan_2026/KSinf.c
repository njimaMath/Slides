...
// T_n = first time an infected frog is at (n,0).
// We run multiple trials, estimate E[T_n] by sample mean, and print:
//   log |T_n - Ehat[T_n]| / log n
//
// Build (GCC/Clang):
//   gcc -O3 -march=native -flto -std=c11 -fopenmp KSinf.c -lm -o KSinf
//
// Example:
//   ./KSinf --n 64 --trials 5 --max-steps 60000 --seed 42
// 'c:\Users\shuta\Documents\Github\Research\Slide\France_Japan_2026\KSinf.exe' --n 5000
#include <math.h>
#include <limits.h>
#include <stdint.h>
#include <stdio.h>
...
// Returns:
// 0 success
// 1 max_steps reached before an infected frog reaches (n,0)
...
static int simulate_one_tn(int32_t n, uint64_t seed, int max_steps, int32_t *t_out) {
    ...
    for (int t = 1; t <= max_steps; t++) {
        ...
        if (posset_contains(&infected_pos, n, 0)) {
            *t_out = t;
...
#pragma omp critical
    ...
    printf("T_n = first time an infected frog is at (n,0)\n");
