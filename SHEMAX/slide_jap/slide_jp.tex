...
$\hat \beta<1$ とする．示したいのは $\log W_N(\beta_N,0) \cvlaw \mathcal N\left(-\frac {\lambda^2} 2,\lambda^2\right)$ ．
\begin{itemize}
...
\begin{itemize}
    ...
    \item 粒子は（独立な）指数分布に従うランダム時刻まで\textbf{ブラウン運動}に従う（{\color{red}指数時計}）．
...
\end{block}
\begin{itemize}
\item<2-> $q\in \mathbb N$ を固定すると，$\hat \beta < 1$ の領域で $W_N$ は全てのモーメントをもつ（Lygkonis--Zygouras 21）．
\item<3-> 改良点：$\hat{\beta_0} \to 1$，$1/3\to 1$，および $o(q^2) \to o(1)$．
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \item 分散 $f(t)$ は，BBMの時間依存拡散係数に対応する．
    \item 不均一BBMの最大値に関する既知の結果（Feng--Zeitouni）より
\[
        \max_{|x|\le 1} \;h_N(x)
        \;=\;
...
{\color{red}{厳密な証明では，十分良い近似のためにモーメント評価に関する良い評価が必要になる．}}
\end{frame}
